  ...
  return (
    <>
            ...
            {log.length > 0 && (
              <small>
                <span style={{ display: "inline-block" }}>
                  {formatDate(new Date(log[log.length - 1].time), "M/DD/YYYY")}
                </span>
                <span style={{ display: "inline-block", marginLeft: "0.25em" }}>
                  {formatDate(new Date(log[log.length - 1].time), "H:mm A")}
                </span>
                <span style={{ paddingLeft: "1em" }}>
                  <small>{log[log.length - 1].message}</small>
                </span>
              </small>
            )}
          </span>
      ...
      >
        ...
        <Offcanvas.Body>
          {log.length < 1 && (
            <div style={{ padding: "0.5rem 1rem" }}>No events</div>
          )}
          {log.map((log, i) => (
            <div
              key={`${log.id}-${i}`}
            ...
            >
              ...
              >
                <small style={{ fontSize: ".875em" }}>
                  <span style={{ display: "inline-block" }}>
                    {formatDate(new Date(log.time), "M/DD/YYYY")}
                  </span>
                  <span style={{ display: "inline-block" }}>
                    {formatDate(new Date(log.time), "H:mm A")}
                  </span>
                </small>
              </span>
