...
import DirectorySelector, {
  DirectorySelection,
} from "../DirectorySelector/DirectorySelector";
...
import {
  ...
  filesystemExtensionMask,
  filesystemInvalidExtensionFamily,
...
const Filesystem = ({ id }: { id: string }) => {
  ...
  const [matchTemplate, setMatchTemplate] = useRecoilState(
    matchTemplateFamily(id)
  );
  const [rewriteTemplate, setRewriteTemplate] = useRecoilState(
    rewriteTemplateFamily(id)
  );
  const [rule, setRule] = useRecoilState(ruleFamily(id));
  const [directorySelection, setDirectorySelection] =
    useRecoilState<DirectorySelection>(directorySelectionFamily(id));
  const [loading, setLoading] = useRecoilState(filesystemLoadingFamily(id));
  const [result, setResult] = useRecoilState<Array<CombyRewriteStatus> | null>(
    filesystemResultFamily(id)
  );
  const [language, setLanguage] = useRecoilState(languageFamily(id));
  const [extensionMask, setExtensionMask] = useRecoilState<string>(
    filesystemExtensionMask(id)
  );
  const [invalidExtensions, setInvalidExtensions] = useRecoilState(
    filesystemInvalidExtensionFamily(id)
  );
  ...
  const onLanguageChange = useCallback(
    (value: string, selected: LanguageOption) => {
      setLanguage((_) => value);
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const results = await invoke<FilesystemResult>(
        "filesystem_rewrite",
        rewrite_args
      );
      if (results.warning) {
        if (results.warning.indexOf("Match rule parse error: :") !== -1) {
          setRuleError(
            results.warning.replace("Match rule parse error: : ", "")
          );
        } else {
  ...
  const apply = useCallback(
    async (filePath: string) => {
      ...
      try {
        ...
        const results = await invoke<FilesystemRewriteFileResult>(
  ...
  const onExtensionChanged = useCallback(
          ...
          >
            ...
            <Form.Group style={{ flexShrink: 1, width: "5em" }}>
              ...
              <Form.Control
                ...
                value={extensionMask}
                size={"sm"}
                onChange={onExtensionChanged}
                isInvalid={invalidExtensions}
              />
            </Form.Group>
          </div>
          <div
          ...
          >
            <Form.Group className="mb-3" controlId="matchTemplate">
              ...
              <Form.Control
                ...
                value={matchTemplate}
                onChange={(e) => setMatchTemplate(sanitize(e.target.value))}
              />
            </Form.Group>
            <Form.Group className="mb-3" controlId="rewriteTemplate">
              ...
              <Form.Control
                ...
                onChange={(e) => setRewriteTemplate(sanitize(e.target.value))}
              />
            </Form.Group>
            <Form.Group className="mb-3" controlId="rule">
              ...
              <InputGroup>
                <Form.Control
                  ...
                  value={rule}
                  onChange={(e) => setRule(sanitize(e.target.value))}
          ...
          {loading ? (
            ...
            <Button
              ...
              disabled={
                invalidExtensions ||
                loading ||
                !Boolean(directorySelection.expanded) ||
                !Boolean(matchTemplate) ||
                !Boolean(rewriteTemplate)
              }
            >
              Run
            </Button>
