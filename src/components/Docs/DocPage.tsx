...
export const unTitleCase = (s: string) =>
  s.replace(/(^[a-z]|-[a-z])/g, (m: string) => {
    if (m.length === 1) {
      ...
      ...old.map((tab) => {
        if (tab.id === params.tabId) {
          return {
            ...
            path: newPath,
            title: `${
              params.pageId !== "" ? unTitleCase(params.pageId) : "Docs"
            }`,
          };
        } else {
          return tab;
            ...
            if (props.href?.indexOf("://") !== -1) {
              return (
                <a {...props} target={"_blank"}>
                  {props.children}
                  <BiWindowOpen style={{ marginLeft: "0.25em" }} />
                </a>
              );
            } else {
