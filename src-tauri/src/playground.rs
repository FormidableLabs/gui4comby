...
#[tauri::command]
pub async fn playground_match(state: tauri::State<'_, DockerState>, app_handle: tauri::AppHandle, source: String, match_template: String, language: String) -> Result<PlaygroundResult, String> {
...
#[tauri::command]
pub async fn playground_rewrite(state: tauri::State<'_, DockerState>, app_handle: tauri::AppHandle, source: String, match_template: String, rewrite_template: String, language: String) -> Result<PlaygroundResult, String> {
    let docker = maybe::maybe_ref(&state.docker)?;
    let result = docker_run::docker_run(docker, vec![
